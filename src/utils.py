...
def log_error_map(preds, trues,  epoch, num_points = 50, TB_writer = None):
    ...
    ax.scatter(trues[:, 1], trues[:, 0], c='lime', label='True', alpha=0.5, s=30, edgecolors='black', zorder=2)
    ax.scatter(preds[:, 1], preds[:, 0], c='red', label='Pred', alpha=0.5, s=30, edgecolors='black', zorder=2)
...
def local_meters_to_latlon(x, y, lat0, lon0):
    R = 6378137.0
